...
def display_prompts(cur, con, search_query, sort_order):
    ...
    for p in prompts:
        with st.expander(f"{'⭐' if p[3] else ''} {p[1]} ", expanded=True):
            ...
            with col1:
                if st.button("Fav⭐", key=f"fav-{p[0]}"):
                    print("favorite")
...
if __name__ == "__main__":
    ...
    # Add a content section for displaying prompts
    content_container = st.container()
    with content_container:
        if view_option == "All Prompts":
            display_prompts(cur, con, search_query, sort_order)
        elif view_option == "Favorites":
            display_prompts(cur, con, search_query, "Favorites")
        elif view_option == "Create New":
